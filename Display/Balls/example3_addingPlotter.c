...
double main( double bc, double v, double sh,double angle, double zero,double windspeed, double windangle, int G , double zeroanglein, double targetDistance, int justangle)
{
	int k=0;
	//double* sln;
	...
	return 3; //(GetPath(sln,50*10) / ((GetRange(sln,50*10)/100)*1.047));		
...
double SolveforAngler( double bc, double v, double sh,double angle, double zero,double windspeed, double windangle, int G , double zeroanglein, double targetDistance, int justangle)
{
...
double HandMeTime(int pos){   x  y  jskj kl jlsakdj f;alksdjf ;alksjdf ;
	return (GetTime(sln,pos));
}
